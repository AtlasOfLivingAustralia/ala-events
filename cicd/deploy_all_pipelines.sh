...
$SCRIPT_DIR/gen_env_vars.py --env $environment  --clean-branch $clean_branch > env.txt
source env.txt
rm env.txt

# deploy/update the template
echo "Deploying the all pipeline templates"

cd base/pipeline
./deploy_pipeline.sh "$@"

cd ../../database/pipeline
./deploy_pipeline.sh "$@"

echo "Waiting for $BASE_STACK_NAME"
if ! aws cloudformation wait stack-create-complete --stack-name $BASE_STACK_NAME; then
  echo "failed to create stack $BASE_STACK_NAME"
  exit
fi
