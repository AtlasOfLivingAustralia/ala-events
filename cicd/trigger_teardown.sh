...
###
# Deploy the codepipeline for the events app
...
$SCRIPT_DIR/gen_env_vars.py --env $environment  --clean-branch $clean_branch --conf $SCRIPT_DIR/frontend/config.ini > env.txt
source env.txt
rm env.txt

# deploy/update the template
echo "Trigger Teardown the all pipeline templates"
...
APPROVAL_TOKEN=
until [ ! -z "$APPROVAL_TOKEN" ]; do
  sleep 5
  APPROVAL_TOKEN=$(aws codepipeline get-pipeline-state \
    --name $PIPELINE_NAME \
    --query "stageStates[?stageName=='Teardown'].actionStates[0].latestExecution.token" \
    --output text)
done 

echo APPROVAL_TOKEN=$APPROVAL_TOKEN

aws codepipeline put-approval-result \
  --pipeline-name $PIPELINE_NAME \
  --stage-name Teardown \
  --action-name Approval \
  --result "summary=CLI trigger teardown,status=Approved" \
